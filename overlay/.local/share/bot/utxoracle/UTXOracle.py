...
#                                                                                       #
#########################################################################################
#                     Version 9.1 RPC Only — Bitcoin on Tails bundled snapshot.
#
# Upstream:  https://utxo.live/oracle/source.php
# Author:    @SteveSimple (Twitter), per UTXOracle License v1.0 (May 2025).
# License:   See LICENSE.txt next to this file. Used here under the
#            "Consensus-Compatible Use" clause for non-commercial integration.
#
# Modifications from upstream made by Bitcoin on Tails:
#   - Replaced the auto-loading YouTube live-stream <iframe> at the bottom of
#     the generated HTML with a plain <a href> link to the same stream. This
#     keeps the license-required link to the UTXOracle Live Stream visible
#     while preventing Tor Browser from auto-fetching YouTube/Google when the
#     local report is opened. Click-through behavior is unchanged.
#   - No changes to price-filtering or averaging logic, per Section 2 of the
#     license. Output names ("UTXOracle Consensus Price" / "UTXOracle Block
#     Window Price") are preserved verbatim.
# If you want a pristine upstream copy, run:
#   curl -O https://utxo.live/oracle/UTXOracle.py
# and verify against upstream's published hash.
...
# NOTE: BoT-bundled snapshot — the upstream UTXOracle.py ends with a YouTube
# live-stream <iframe>, which would leak the user's Tor circuit to YouTube/Google
# on Tails. We removed it here. To restore upstream behavior, fetch a fresh copy
# from https://utxo.live/oracle/UTXOracle.py.
...
<!--
  License compliance: the UTXOracle License v1.0 requires that integrations
  display "a link to the UTXOracle YouTube Live Stream". Bitcoin on Tails
  bundles this snapshot for use on Tails OS, where loading a YouTube iframe
  would auto-fetch Google over the user's Tor circuit. We satisfy the
  license by rendering a plain <a> link instead — the user has to click it
  to open the stream, no auto-load. Same target URL as upstream.
-->
<h2 style="margin-top:10px; font-size:24px; color:#CCCCCC;">
  Want a
  <span style="color:orange;">Live Updating Oracle</span>
  with new mempool transactions?
</h2>
<p style="margin: 0 auto 30px; font-size: 18px;">
  <a
    href="https://www.youtube.com/channel/UCXN7Xa_BF7dqLErzOmS-B7Q/live"
    rel="noopener noreferrer"
    target="_blank"
    style="color:cyan; text-decoration:underline;"
  >Open the UTXOracle Live Stream on YouTube &#x2192;</a>
  <br>
  <span style="color:#888888; font-size:14px;">
    (External link — opens in a new tab. Loading YouTube exposes your IP /
    Tor circuit to Google.)
  </span>
</p>
